...
const HouseListContent = () => {
    ...
    const handleAddOrUpdate = async () => {
        ...
        setLoading(true);
        try {
            // Only include landlord if not empty, else set to null
            const landlordValue = form.landlord && form.landlord.trim() !== '' ? form.landlord.trim() : null;
            const houseData = {
                name: form.name.trim(),
                ...
                ...(landlordValue !== null && { landlord: landlordValue })
            };
            if (editingId) {
            ...
            console.error('Failed to save house:', error, error?.response?.data || error?.message);
    ...
    const handleDelete = (id, houseName) => {
        if (!user || !['admin', 'manager'].includes(user.role)) {
            Alert.alert('Permission Denied', 'Only admins or managers can delete houses.');
            return;
        }
        console.log('Delete button pressed for house:', id, houseName);
        Alert.alert('Debug', 'Delete confirmation dialog should appear next.');
        Alert.alert(
            'Delete House',
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    style: 'destructive',
                    onPress: async () => {
                        Alert.alert('Debug', 'Delete confirmation button pressed!');
                        console.log('Delete confirmation pressed for house:', id, houseName);
                        setLoading(true);
                        try {
                            ...
                            console.error('Failed to delete house:', error, error?.response?.data || error?.message);
                    ...
                    >
                        ...
                        <View style={[dynamicStyles.tableCell, dynamicStyles.actionsCell]}>
                            <IconButton
                                ...
                                accessibilityLabel={`Edit ${house.name}`}
                            />
                            <IconButton
                                ...
                                accessibilityLabel={`Toggle occupancy for ${house.name}`}
                            />
                            <IconButton
                                icon="delete"
                                size={20}
                                ...
                                accessibilityLabel={`Delete ${house.name}`}
                                disabled={!user || !['admin', 'manager'].includes(user.role)}
                            />
                        </View>
                    </View>
                ))}
