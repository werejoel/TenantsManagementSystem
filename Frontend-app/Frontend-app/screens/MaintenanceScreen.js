...
const MaintenanceScreen = () => {
  ...
  const saveEdit = async (id) => {
    try {
      ...
      await axios.patch(
        ...
        { status: editStatus, notes: editNotes, reply },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setEditingId(null);
      fetchRequests();
      Alert.alert('Success', 'Request updated and notification sent.');
    } catch (error) {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.label}>Reply/Notification:</Text>
      {editingId === item.id ? (
        <TextInput
          style={styles.input}
          value={reply}
          onChangeText={setReply}
          placeholder="Type a reply or notification to tenant..."
          multiline
        />
      ) : null}
      ...
      {editingId === item.id ? (
        <TouchableOpacity style={styles.saveButton} onPress={() => saveEdit(item.id)}>
          <Text style={styles.saveButtonText}>Save & Notify</Text>
        </TouchableOpacity>
      ) : (
